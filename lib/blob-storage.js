...
export async function uploadToBlob(file, options = {}) {
  ...
  try {
    // Get the original filename from the file object or options
    let originalFilename = 'image.jpg';
    
    // If a filename was provided in the options, use that
    if (providedFilename) {
      originalFilename = providedFilename;
    }
    // If the file is a Blob with a name property (set by the upload handler)
    else if (file.name) {
      originalFilename = file.name;
    } 
    // If the file is a File object (from a form upload)
